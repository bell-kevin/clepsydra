...
/**
 * Where the numbers on screen came from. Before anything is fitted the app
 * still has to draw a curve somewhere, and it uses the Cooper-Jacob
 * straight-line estimate. That is a real method and a reasonable number, but
 * printing it under the same heading as a fitted result, in the same size and
 * colour, invites it to be read as the answer. It is labelled instead.
 */
export type ParamSource = 'fit' | 'manual' | 'seed';
...
export function Readout({ setup, params, result, source, transmissivityUnit }: Props) {
  const tUnit = findUnit(TRANSMISSIVITY_UNITS, transmissivityUnit);
  const showT = Number.isFinite(params.T);
  const errors = source === 'fit' ? (result?.errors ?? {}) : {};
    ...
    <>
      <div className="readout">
        <div>
          ...
          <div className="result-value" data-provisional={source !== 'fit'}>
        ...
        {setup.model === 'hantush' && (
          <div>
            ...
            <div className="result-value" data-provisional={source !== 'fit'}>
              {params.rOverB ? sig(params.rOverB) : '—'}
            </div>
      ...
      {source === 'seed' && (
        <p className="provenance">
          <b>Starting estimate, not a fit.</b> This is the Cooper–Jacob straight line through the
          later readings, which is what the curve is drawn from before anything is matched. Press{' '}
          <b>Match the curve</b> to fit all four decades properly, or drag the curve yourself.
        </p>
      )}
