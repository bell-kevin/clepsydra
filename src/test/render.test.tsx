...
describe('the model curve stays inside the ruled area', () => {
  /**
   * Theis drawdown tends to zero as time tends to zero, so on log paper the
   * type curve descends without limit. It was drawing straight out of the
   * bottom of the plot box and over the axis labels.
   */
  it('clips the curve group and puts the clip on the plot rectangle', () => {
    ...
    const clip = markup.match(/<clipPath id="([^"]+)"><rect x="(\d+)" y="(\d+)" width="(\d+)" height="(\d+)"/);
    expect(clip).not.toBeNull();
    const [, id, x, y, w, h] = clip!;
    // The clip must cover the ruled rectangle exactly, not the whole canvas.
    expect(markup).toContain(`clip-path="url(#${id})"`);
    expect(Number(x)).toBe(66);
    expect(Number(y)).toBe(22);
    expect(Number(w)).toBe(808);
    expect(Number(h)).toBe(492);
  });

  it('the curve really does run past the plot edge, so the clip is doing work', () => {
    const setup = SAMPLES[0].setup;
    const markup = renderToStaticMarkup(
      <Sheet
        setup={setup}
    ...
    const paths = Array.from(
      markup.matchAll(/<path d="([^"]+)" fill="none" stroke="var\(--model\)"/g),
    ).map((m) => m[1]);
    expect(paths.length).toBe(2);
    const ys = paths.flatMap((path) =>
      Array.from(path.matchAll(/[ML]([\d.]+) ([\d.]+)/g)).map((m) => Number(m[2])),
    );
    expect(ys.length).toBeGreaterThan(0);
    // Below y = 514 is outside the ruled area. It is the more distant
    // piezometer whose curve dives off the bottom, so both are checked.
    // If this ever stops being true the clip has become decorative and this
    // test should be reconsidered, not deleted.
    expect(Math.max(...ys)).toBeGreaterThan(514);
  });
});
